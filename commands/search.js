...
exports.run = async (client, msg, args) => {
	...
	if (!args[1]) return msg.channel.send({ embed: { color: 0x4286f4, description: `**${prefix}search** ***[Song Name]***`}});
	...
	if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
		...
		return msg.channel.send({ color: 0x06238B, embed: { description: `<:m_cek:500912712222507008> Playlist: **${playlist.title}** has been added to the queue!`}});
	} else {
		try {
